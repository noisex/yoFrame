...
function addHealPred( self)
	...
	self:RegisterEvent("UNIT_HEAL_PREDICTION", healthUpdate)
	self:RegisterEvent("UNIT_HEAL_ABSORB_AMOUNT_CHANGED", healthUpdate)
	return healPred
end

function healthUpdate( f, event, unit)
	local thText
...
local function unitShared(self, unit)
	...
	if unit == "player" or unit == "target" or unit == "pet" then
		self.Health.AbsorbBar = addAbsorbBar( self)
		self.Health.healPred  = addHealPred( self)
