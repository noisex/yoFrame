...
HealthPred - A `table` containing references to sub-widgets and options.
...
local function Update(self, event, unit)
	...
	local element = self.HealthPred
	--[[ Callback: HealthPred:PreUpdate(unit)
	Called before the element has been updated.

	* self - the HealthPred element
...
--	local hasOverHealAbsorb = false
	...
	--	element.myBar:Show()
	--end
	--print( health, allIncomingHeal, absorb)

	if(element.otherBar) then
		element.otherBar:SetMinMaxValues(0, maxHealth)
		element.otherBar:SetValue( min( allIncomingHeal, maxHealth - health))
		element.otherBar:Show()
	end

	if(element.absorbBar) and health > absorb then
		element.absorbBar:SetMinMaxValues(0, maxHealth)
		element.absorbBar:SetValue( min( absorb, maxHealth - health))
		element.absorbBar:Show()
	end

	if(element.healAbsorbBar) then
		element.healAbsorbBar:SetMinMaxValues(0, maxHealth)
		element.healAbsorbBar:SetValue( min( healAbsorb, health))
	...
	--[[ Callback: HealthPred:PostUpdate(unit, myIncomingHeal, otherIncomingHeal, absorb, healAbsorb, hasOverAbsorb, hasOverHealAbsorb)
...
local function Path(self, ...)
	--[[ Override: HealthPred.Override(self, event, unit)
	...
	* unit  - the unit accompanying the event
	--]]
	return (self.HealthPred.Override or Update) (self, ...)
end
...
oUF:AddElement('HealthPred', Path, Enable, Disable)
