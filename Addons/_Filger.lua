...
function Filger:DisplayActives()
		...
		if not bar then
			...
			if self.Mode == "ICON" then
				...
				if bar.statusbar then
					...
					bar.statusbar:SetWidth( C.filger.barWidth) 
					bar.statusbar:SetHeight(C.filger.barHeight)
				...
				if bar.background then
					...
					bar.background:SetTexture( texture)
					bar.background:SetVertexColor( 0.09, 0.09, 0.09, 0.2)
				end
				...
				if bar.time then
					...
					bar.time:SetPoint("RIGHT", bar.statusbar, 0, 1)
	...
	local activeCount = 1
	local limit = (25 * 12)/self.IconSize
